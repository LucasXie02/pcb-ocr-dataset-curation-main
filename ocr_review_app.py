...
_LAST_DATASET_FILE = Path(__file__).parent / '.ocr_last_dataset'

def _save_last_dataset_root(dataset_root: Path):
    try:
        _LAST_DATASET_FILE.write_text(str(dataset_root), encoding='utf-8')
    except Exception:
        pass

def _load_last_dataset_root() -> Optional[Path]:
    try:
        if _LAST_DATASET_FILE.exists():
            p = Path(_LAST_DATASET_FILE.read_text(encoding='utf-8').strip())
            if p.exists():
                return p
    except Exception:
        pass
...
def api_accept_line():
    ...
    try:
        ...
        result = find_line_in_annotations(line_uid, app_state['annotations'])
        if not result:
            for anns in app_state.get('all_annotations', {}).values():
                result = find_line_in_annotations(line_uid, anns)
                if result:
                    break
        if not result:
            return jsonify({'success': False, 'error': 'Line not found'})
...
def api_edit_line():
    ...
    try:
        ...
        # Find line in annotations (search current subdir first, then all subdirs as fallback)
        result = find_line_in_annotations(line_uid, app_state['annotations'])
        if not result:
            for anns in app_state.get('all_annotations', {}).values():
                result = find_line_in_annotations(line_uid, anns)
...
def api_finish_review():
    ...
    try:
        ...
        # Rebuild component groups so gallery reflects the updated candidate/ contents
        load_manifest_and_groups()
        invalidate_metrics()
...
def api_set_dataset_root():
    ...
    try:
        ...
        # Persist dataset root so it auto-loads on next server restart
        _save_last_dataset_root(dataset_root)
...
def main():
    ...
    # If no --dataset_root provided, try to restore the last used one
    if not args.dataset_root:
        restored = _load_last_dataset_root()
        if restored:
            logging.info(f"Auto-restoring last dataset root: {restored}")
            args.dataset_root = str(restored)
